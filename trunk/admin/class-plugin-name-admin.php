...
class Plugin_Name_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script ( $this->plugin_name, plugin_dir_url ( __FILE__ ) . 'js/plugin-name-admin.js', array (
				'jquery' 
		), $this->version, false );
	}
	...
	public function carousel_meta_box() {
		add_meta_box ( 'wooslickcarousel_meta_box', __ ( 'Slick Carousel Settings', $this->plugin_name ), array (
				$this,
				'render_carousel_meta_box_content' 
		), 'wooslickcarousel', 'advanced', 'high' );
	}
	public function render_carousel_meta_box_content($post) {
		 ...
		 * echo '</ul>';
		 */
		$xx = Carousel::loadByID( $this->plugin_name, $this->version, $post->ID );
	...
	public function save($post_id) {
		...
		$carousel = Carousel::loadByID ( $this->plugin_name, $this->version, $post_id );
	...
	public function shortcode($atts) {
		...
		if (sizeof ( $carousel_id ) > 0) {
			...
			$carousel = Carousel::loadByID ( $this->plugin_name, $this->version, $atts ['id'] );
			$slick_arg = $carousel->get_owlc_js_arg();
